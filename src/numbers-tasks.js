...
function getAverage(value1, value2) {
  const valOne = value1 / 2;
  const valTwo = value2 / 2;
...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const lineY = (y1 - y2) ** 2;
  const lineX = (x1 - x2) ** 2;
  const result = Math.sqrt(lineY + lineX);
  return result;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const rootOne = Math.sqrt(x1 * x1 + y1 * y1);
  const rootTwo = Math.sqrt(x2 * x2 + y2 * y2);
  return Math.acos(line / (rootOne * rootTwo));
}
...
function getParallelepipedDiagonal(a, b, c) {
  const sumParties = a ** 2 + b ** 2 + c ** 2;
  return Math.sqrt(sumParties);
}
